...
|___/_|_|\___|_|\_(_)/ |___/
 ...
 Version: 1.6.1
  09-12-2016 -	Modified by Nassos Yiannopoulos
				* added 'fitToSlides', 'alignCenter'. they are true by default
				* when fitToSlides=false it distributes the width of each slide 
					so that slidesToShow slides are always visible.
				* changed how the carousel width is calculated when variableWidth==true
...
(function(factory) {
	...
	Slick.prototype.getLeft = function(slideIndex) {
		...
		if (_.options.vertical === false) {
			if (_.options.alignCenter)
			{
				var slideToShowWidth = _.slideWidth * _.options.slidesToShow;
				targetLeft = ((slideIndex * _.slideWidth - 0.5*(_.listWidth - slideToShowWidth)) * -1) + _.slideOffset;
			}
			else
			{
	...
	Slick.prototype.getNavigableIndexes = function() {
		...
		while (breakPoint < max) {
			...
			if (guard < 0)
			{
				console.log('getNavigableIndexes guard 100 times (breakPoint['+breakPoint+'] < max['+max+'])');
				guard = 100;
			}
		}
	...
	Slick.prototype.refresh = function( initializing ) {
		...
		// $.extend(_, _.initials, _.options, { currentSlide: currentSlide });
		// $.extend(_, _.options, { currentSlide: currentSlide });
	...
	Slick.prototype.setDimensions = function() {
		...
		if (_.options.vertical === false && _.options.variableWidth === false) {
			if (_.options.fitToSlides)
			{
				_.slideWidth = Math.ceil(_.$slideTrack.children('.slick-slide').first().outerWidth(true));
			...
			var width = 0;
			_.$slideTrack.children('.slick-slide').each(function(index, el)
			{
				width += Math.ceil($(el).outerWidth(true) || 0);
			});
	...
	Slick.prototype.calcIndex = function() {
		var _ = this;
		var m = _.$slideTrack.css('transform') || '0, 0, 0, 0, 0, 0';
		m = m.split(',');
		var xOffset = m[4];
	...
	Slick.prototype.slideHandler = function(index, sync, dontAnimate) {
		...
		if ( _.options.asNavFor ) {
			...
			if ( navTarget && navTarget.slideCount <= navTarget.options.slidesToShow ) {
